...
class DownSamplingBlock(nn.Module):
    def __init__(self, in_channel, out_channel, dropout=0.0):
        ...
        layers = [nn.LeakyReLU(0.2),
                  nn.utils.spectral_norm(nn.Conv2d(in_channel, out_channel, (4,4), (2,2), (1,1), bias=False)),
...
class UpSamplingBlock(nn.Module):
    def __init__(self, in_channel, out_channel, dropout=0.0):
        ...
        layers = [
                  nn.LeakyReLU(0.2),
                  nn.utils.spectral_norm(nn.ConvTranspose2d(in_channel, out_channel, (4,4), (2,2), (1,1), bias=False)),
                  nn.InstanceNorm2d(out_channel, affine=True)
                  ]
        if dropout > 0.0:
            layers.append(nn.Dropout(dropout))
...
class StandardUnit(nn.Module):
    def __init__(self, in_channel, out_channel, dropout=0.0):
        ...
        layers = [nn.ConstantPad2d((1, 2, 1, 2), 0), nn.utils.spectral_norm(nn.Conv2d(in_channel, out_channel, (4,4), (1,1)))]
        if dropout > 0.0:
            layers.append(nn.Dropout(dropout))
        layers.append(nn.ConstantPad2d((1, 2, 1, 2), 0))
        layers.append(nn.utils.spectral_norm(nn.Conv2d(out_channel, out_channel, (4,4), (1,1), bias=False)))
...
class FinalUnetActivate(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        layers = [
                  nn.LeakyReLU(0.2),
                  nn.utils.spectral_norm(nn.ConvTranspose2d(in_channel, out_channel, (4,4), (2,2), (1,1))),
                  nn.Tanh()
                  ]
        self.model = nn.Sequential(*layers)
...
class AttUNetPlusPlus(nn.Module):
    def __init__(self, in_channel, ngf):
        ...
        self.conv1_0 = nn.utils.spectral_norm(nn.Conv2d(in_channel, ngf, (4,4), (2,2), (1,1)))
        self.conv1_1 = nn.utils.spectral_norm(nn.Conv2d(ngf, ngf, (4,4), (2,2), (1,1)))
        ...
        self.FUA_mag_in = UpSamplingBlock(ngf, ngf//2)
        self.FUA_phase_in = UpSamplingBlock(ngf, ngf // 2)

        self.FUA_mag = FinalUnetActivate(ngf//2, 1)
        self.FUA_phase = FinalUnetActivate(ngf//2, 2)

    def forward(self, complex_stft, mag_stft):                       # 256 x 256
        x = torch.cat([complex_stft, mag_stft], 1)
        conv1_0 = self.conv1_0(x)
        conv1_1 = self.conv1_1(conv1_0)               # 128 x 128
        ...
        conv2_5 = torch.cat((up2_5, conv2_1, conv2_2, conv2_3, conv2_4, down2_5), dim=1)
        conv2_5 = self.conv2_5(conv2_5)
        conv1_5, mask1_5 = self.ga1_6(conv1_5, conv2_5)
        up1_6 = self.up1_6(conv2_5)
        conv1_6 = torch.cat((up1_6, conv1_1, conv1_2, conv1_3, conv1_4, conv1_5), dim=1)
        conv1_6 = self.conv1_6(conv1_6)

        mag_in = self.FUA_mag_in(conv1_6)
        phase_in = self.FUA_phase_in(conv1_5)

        out_mag = self.FUA_mag(mag_in)
        out_complex = self.FUA_phase(phase_in)
